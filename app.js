    ...
    libraryItems.forEach((item) => {
      item.addEventListener('click', (event) => {
        if (event.target.classList.contains('library-item')) {
          ...
          isPlaying = false;
          playTrack(isPlaying);
          nowPlaying(event.target.children[2].innerText, event.target.children[3].innerText);
...
async function searchTrack() {
  container.innerHTML = '';
  const fetchData = await fetch(`https://api.napster.com/v2.2/search?query=${query}&per_type_limit=50`, {
    method: 'GET',
    headers: {
      apikey: key,
    },
  ...
  itemContainers.forEach((itemContainer) => {
    ...
    addButton.addEventListener('click', (event) => {
      ...
      createLibraryItem(artist, title, src, JSON.parse(localStorage.getItem('songs')).length - 1);
    });
  });
}
